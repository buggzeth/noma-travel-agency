...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = `
      ...
      1. DESTINATION & VIBE: Identify the primary destination strictly from the video content. Craft a brief, engaging summary for the destination (max 2 sentences).
      ...
      4. ITINERARY RULES: Match the exact duration requested (${days} days). Fill the itinerary strictly with exploring, dining, and activities matching their Preferred Pace (${pace}). Structure the day realistically by providing up to 3 separate activities, taking into account the 'estimatedTime' of each so that all activities can comfortably fit into one day. Provide a separate section for distinct 'breakfast', 'lunch', and 'dinner' plans. Every activity and meal MUST mention specific, real places. Assume transit logistics are handled. Group all activities and meals for each individual day so they are geographically clustered close to each other (walking distance or a very short ride). Avoid scattering locations across the destination in a single day to ensure a seamless, highly convenient travel experience.
